...
public class ExecutionController {
    ...
    public ResponseEntity<byte[]> logTail(@PathVariable Long id,
        ...
        // 这段窗口里「行」和「许可」都还不存在 —— 这属于「还没有日志」，
        // 不是「执行不存在」。不区分就会返回 404，在浏览器里表现为一条无意义的
        // 控制台报错（下一次轮询自愈，但噪音会污染回归点检）。
        if (h == null) {
            ExecutionGate.Permit live = executionGate.get(id);
            boolean runningNow = live != null && !live.finished();
            if (runningNow || executionGate.isAllocated(id)) {
